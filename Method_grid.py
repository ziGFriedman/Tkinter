...
'''С помощью методов grid_remove() и grid_forget() можно сделать виджет невидимым.
Отличие между этими методами лишь в том, что grid_remove() запоминает прежнее положение
виджета. Поэтому для его отображения в прежней ячейки достаточно применить grid() без
аргументов. После grid_forget() потребуется заново конфигурировать положение виджета.

Скрытие виджетов бывает необходимо в тех случаях, когда, например, от выбора
пользователя в одной части интерфейса зависит, какие виджеты появятся в другой.'''
